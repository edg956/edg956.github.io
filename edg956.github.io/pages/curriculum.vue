<template>
  <div class="container">
    <section class="section">
      ...
      <div class="content">
      	<b-tag v-for="tag in weightedTags" class="mr-1" rounded>{{tag}}</b-tag>
      </div>
    </section>
    <section class="section" v-for="job in experience">
      ...
      <div class="content">
      	<p>{{job.description}}</p>
	<b-tag v-for="tag in job.tags" class="mr-1" rounded>{{tag}}</b-tag>
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
	  ...
	  tags: ['Python', 'Django', 'Docker', 'CI/CD', 'DevOps', 'AWS']
	},
      	{
	  ...
	  startDate: '2022-01',
	  endDate: 'Now',
	  description: 'Used Django REST framework to develop an API for the onboarding process of a digital wallet, including KYC, AML and identity verification. Managed applications deployed in GKE with ArgoCD and Helm. Read and applied minor patches in Go codebases as required because of the microservice architecture of the company',
